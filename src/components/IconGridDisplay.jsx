...
import {
  ...
  FaPython, FaBootstrap, FaDatabase,
} from "react-icons/fa";
import { FaTableList } from 'react-icons/fa6'; // Assuming FaTableList is from fa6
import {
  SiExpress, SiTailwindcss, SiMongodb, SiFirebase,
  SiNextdotjs, SiMysql, SiPostgresql, SiFastapi, SiLangchain,
} from "react-icons/si";
...
const allAvailableIcons = [
  ...
  { icon: <SiFastapi className="text-red-600" /> },
  { icon: <SiPostgresql className="text-blue-600" /> },
  { icon: <SiLangchain className="text-blue-400" /> },
  { icon: <SiMysql className="text-yellow-400" /> },
  ...
  { icon: <FaTableList className="text-blue-300" /> },
];
...
export default function IconGridDisplay({ contentRect = null }) {
  ...
  const getRandomPointInBounds = (
    minAllowedX, maxAllowedX, minAllowedY, maxAllowedY, // Area to place the icon in
    ...
    while (attemptsLeft > 0) {
      ...
      if (excludeRect &&
          testX > excludeRect.left && testX < (excludeRect.left + excludeRect.width) &&
          testY > excludeRect.top && testY < (excludeRect.top + excludeRect.height)) {
        attemptsLeft--;
  ...
  const getNonOverlappingPosition = (
    ...
    targetAreaMinX, targetAreaMaxX, targetAreaMinY, targetAreaMaxY
  ) => {
    ...
    const actualMaxX = Math.min(targetAreaMaxX, containerWidth - boundaryPadding);
    const actualMaxY = Math.min(targetAreaMaxY, containerHeight - boundaryPadding);

    // If the target area is too small after accounting for padding, return null
    if (actualMaxX <= actualMinX || actualMaxY <= actualMinY) {
        console.warn("Target placement area too small or invalid after padding/bounds adjustment.");
        return null;
    }

    while (attempts < MAX_PLACEMENT_ATTEMPTS) {
        // Get a random point within the current target area and avoiding the content exclusion
        const testPoint = getRandomPointInBounds(actualMinX, actualMaxX, actualMinY, actualMaxY, contentExclusionRect, MAX_PLACEMENT_ATTEMPTS - attempts);
        if (!testPoint) {
        ...
        for (const existing of allPlacedIcons) {
            const distance = Math.sqrt(
                Math.pow(testPoint.x - existing.left, 2) + Math.pow(testPoint.y - existing.top, 2)
            );
            if (distance < MIN_ICON_SPACING_PX) { // Check against the new MIN_ICON_SPACING_PX
                overlaps = true;
  ...
  useEffect(() => {
    const updateIconsAndPositions = () => {
      ...
      } else if (currentWidth >= 768 && currentWidth < 1024) { // Tablet
      ...
      const shuffledIcons = [...allAvailableIcons].sort(() => 0.5 - Math.random());
      ...
      if (currentIsDesktop) {
        ...
        const desktopPlacementZones = [
          ...
          { minX: 0, maxX: containerRect.width / 3, minY: cornerOffset, maxY: containerRect.height / 3 + cornerOffset, count: 2 },
          // Upper-Right (2 icons) - Shifted down slightly (minY increased)
          { minX: containerRect.width * 2 / 3, maxX: containerRect.width, minY: cornerOffset, maxY: containerRect.height / 3 + cornerOffset, count: 2 },
          // Lower-Left (2 icons) - Shifted up slightly (maxY decreased)
          { minX: 0, maxX: containerRect.width / 3, minY: containerRect.height * 2 / 3 - cornerOffset, maxY: containerRect.height - cornerOffset, count: 2 },
          // Lower-Right (2 icons) - Shifted up slightly (maxY decreased)
          { minX: containerRect.width * 2 / 3, maxX: containerRect.width, minY: containerRect.height * 2 / 3 - cornerOffset, maxY: containerRect.height - cornerOffset, count: 2 },
          // Upper-Center (2 icons)
          { minX: containerRect.width / 3, maxX: containerRect.width * 2 / 3, minY: 0, maxY: containerRect.height / 4, count: 2 },
          // Lower-Center (2 icons)
          { minX: containerRect.width / 3, maxX: containerRect.width * 2 / 3, minY: containerRect.height * 3 / 4, maxY: containerRect.height, count: 2 },
        ];
        ...
        for (const zone of desktopPlacementZones) {
          for (let i = 0; i < zone.count; i++) {
            ...
            const newPos = getNonOverlappingPosition(
              ...
              zone.minX, zone.maxX, zone.minY, zone.maxY // Target specific zone
            );

            if (newPos) {
              newRandomIconsWithPositions.push({
                id: `${shuffledIcons[iconIndex].icon.props.className}-${zone.minX}-${zone.minY}-${i}-${new Date().getTime()}`,
                icon: shuffledIcons[iconIndex].icon,
                ...newPos,
              });
              iconIndex++;
            } else {
              console.warn(`Could not place icon in zone (${zone.minX}, ${zone.minY}) after ${MAX_PLACEMENT_ATTEMPTS} attempts.`);
            }
          }
        }

      } else { // Mobile & Tablet: random scatter across entire screen
        ...
        for (let i = 0; i < selectedIcons.length; i++) {
          const newPos = getNonOverlappingPosition(
            ...
            0, containerRect.width, 0, containerRect.height // Target entire container
          );

          if (newPos) {
            newRandomIconsWithPositions.push({
              id: `${selectedIcons[i].icon.props.className}-${i}-${new Date().getTime()}`,
        ...
        return (
          <motion.div
            ...
            animate={{
              ...
              rotate: [item.rotate, item.rotate + (Math.random() * 10 - 5), item.rotate], // Rotates by -5 to +5 degrees
            }}
            transition={{
